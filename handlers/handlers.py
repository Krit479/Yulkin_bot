...
async def send_video(message: Message, file_name: str, reply_markup=None):
    ...
    logger.info('Sending video %s (%0.2f MB)', video_path, video_path.stat().st_size / (1024*1024))
    try:
        ...
        logger.exception('TelegramNetworkError while sending video %s', video_path)
        await message.answer(f'Ошибка при отправке видео: {e}. Попробуйте позже.')
    except Exception as e:
        logger.exception('Unexpected error while sending video %s', video_path)
...
async def process_start_command(message: Message):
    await message.answer(
        LEXICON['/start'], disable_web_page_preview=True
    )
...
async def process_continue_btn2_callback(callback: CallbackQuery):
    ...
    await callback.message.answer(
        LEXICON['/continue2'],
        reply_markup=create_inline_kb(1, 'continue_btn3')
    )
